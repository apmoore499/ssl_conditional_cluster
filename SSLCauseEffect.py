...
def disc_eff_soft_EM(x_c, y, x_e, z_c, z_y, z_e,tol,timeout):
    """
    This function implements an EM style algorithm to fit a generative model with categorical, rather than continuous
     effect features.
    :param x_c: causal features of labelled data
    ...
    st=time.time()
    et=time.time()
    tdif=et-st #for timeout given in seconds
    while not converged and tdif<timeout:
        # compute weights
        ...
        if max(np.abs(z_y_soft - z_y_soft_new)) < tol:
            converged = True
...
def run_methods(x_c, y, x_e, z_c, z_y, z_e,tol,timeout):
    x = np.concatenate((x_c, x_e), axis=1)
    z = np.concatenate((z_c, z_e), axis=1) #z is actual labels
    ...
    # EM with soft labels
    a_y, b_y, a_e0, a_e1, b_0, b_1, cov_e0, cov_e1 = soft_label_EM(x_c, y, x_e, z_c, z_e,tol,timeout)
